...
import { todayIsoLocal } from '@/lib/room-status';
import { cn } from '@/lib/utils';

export type StayDayKind = 'available' | 'reserved' | 'occupied' | 'unavailable';
...
/** Per-day kind for the selected room. */
export function stayDayKind(
  dayIso: string,
  stays: OccupancyStay[],
  opts: { today?: string; roomUnavailable?: boolean },
): StayDayKind {
  if (opts.roomUnavailable) return 'unavailable';
  const today = opts.today ?? todayIsoLocal();
  const covering = stays.find((s) => s.check_in <= dayIso && dayIso < s.check_out);
  if (!covering) return 'available';
  // Upcoming confirmed stay nights → reserved; ongoing stay nights → occupied
  if (covering.check_in > today) return 'reserved';
  return 'occupied';
}

type StayAvailabilityCalendarProps = {
  checkIn: string;
  checkOut: string;
  stays: OccupancyStay[];
...
export function StayAvailabilityCalendar({
  ...
  const selected: DateRange | undefined = useMemo(() => {
    ...
    return {
      from: localDate(checkIn),
      to: checkOut ? localDate(checkOut) : undefined,
    };
  }, [checkIn, checkOut]);
  ...
  const occupiedMatchers = useMemo(() => {
    ...
    for (const stay of stays) {
      ...
      while (cursor < end) {
        ...
        if (stayDayKind(iso, stays, { today, roomUnavailable }) === 'occupied') {
  ...
  const reservedMatchers = useMemo(() => {
    ...
    for (const stay of stays) {
      ...
      while (cursor < end) {
        const iso = toIso(cursor);
        if (stayDayKind(iso, stays, { today, roomUnavailable }) === 'reserved') {
          dates.push(new Date(cursor));
        }
        cursor = new Date(cursor);
        cursor.setDate(cursor.getDate() + 1);
  ...
  const isBookedIso = (iso: string) => {
    const kind = stayDayKind(iso, stays, { today, roomUnavailable });
  ...
  const handleSelect = (range: DateRange | undefined) => {
    ...
    if (!range.to || toIso(range.to) === fromIso) {
      onChange({ checkIn: fromIso, checkOut: '' });
      return;
    }

    const toIsoStr = toIso(range.to);
    // Walk nights [from, to) — checkout day itself may be free
    let cursor = localDate(fromIso);
    const end = localDate(toIsoStr);
    while (cursor < end) {
      if (isBookedIso(toIso(cursor))) {
        // Snap check-out to first conflict day (valid half-open end)
        onChange({ checkIn: fromIso, checkOut: toIso(cursor) });
        return;
      }
      cursor = new Date(cursor);
      cursor.setDate(cursor.getDate() + 1);
    ...
    <div className={cn('space-y-2', className)}>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            type="button"
            className="flex w-full items-center justify-between rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-left text-white transition-colors hover:border-accent focus:outline-none focus:border-accent"
          >
            <span className={cn(!checkIn && 'text-white/45')}>{label}</span>
        ...
        >
          <div className="border-b border-white/10 px-4 py-3">
            <p className="text-sm font-semibold text-white">Choose your stay</p>
            <p className="mt-0.5 text-xs text-white/50">
              Green = available · Blue = reserved · Amber = occupied
            </p>
          </div>
          ...
          <DayPicker
            ...
            defaultMonth={checkIn ? localDate(checkIn) : todayDate}
            disabled={[
              { before: todayDate },
              ...(roomUnavailable ? [{ from: todayDate, to: new Date(2100, 0, 1) }] : []),
              ...occupiedMatchers,
              ...reservedMatchers,
            ]}
            modifiers={{
              available: (date) => {
                ...
                return stayDayKind(iso, stays, { today, roomUnavailable }) === 'available';
              },
          ...
          <div className="flex items-center justify-between border-t border-white/10 px-4 py-2.5">
            ...
            <button
              ...
              className="text-xs font-semibold text-sky-300 hover:text-sky-200"
              onClick={() => {
                onChange({ checkIn: today, checkOut: '' });
              }}
            >
              Today
            </button>
      ...
      <div className="flex flex-wrap gap-3 text-[11px] font-semibold text-white/60">
        ...
        <span className="inline-flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-sm bg-amber-400/80" /> Occupied
        </span>
        {roomUnavailable && (
          <span className="inline-flex items-center gap-1.5">
      ...
      {/* Hidden required fields for native form validation */}
